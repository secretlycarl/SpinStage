#!/usr/bin/env python3
"""Stage config/user-settings.json into www/ before cap sync."""
...
LEGACY = ROOT / "www" / "config" / "user-settings.json"
DEST = ROOT / "www" / "config" / "user-settings.json"
...
def resolve_source() -> tuple[Path | None, dict | None]:
    ...
    if LEGACY.is_file():
        return LEGACY, json.loads(LEGACY.read_text(encoding="utf-8"))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage user-settings.json for Android build")
