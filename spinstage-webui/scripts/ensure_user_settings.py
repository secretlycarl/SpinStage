#!/usr/bin/env python3
"""Prompt to create config/user-settings.json when missing (webui first-run helper)."""
...
DEST = WEBUI / "config" / "user-settings.json"
...
def main() -> None:
    ...
    if DEST.is_file() and args.force:
        print(f"Updating {DEST.relative_to(WEBUI)}")
    else:
        print(f"No config at {DEST.relative_to(WEBUI)}")
    ...
    print(f"Wrote {DEST.relative_to(WEBUI)}")
